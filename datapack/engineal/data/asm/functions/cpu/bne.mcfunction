execute if score debug cpu matches 1.. run tellraw @p [{"text":"bne "},{"score":{"name":"rs","objective":"cpu"}},{"text":", "},{"score":{"name":"rt","objective":"cpu"}},{"text":", "},{"score":{"name":"immediate","objective":"cpu"}}]
...
execute unless score value1 alu = value2 alu run scoreboard players operation pc cpu += tmp_val cpu
