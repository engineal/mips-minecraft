...
execute unless score value mem matches 0 run function hardware:vga/char

execute at @e[name=char_pos] run teleport @e[name=char_pos] ~-6 ~ ~
scoreboard players add $a0 reg 1
execute unless score value mem matches 0 run function hardware:cpu/special/syscall/print_string
