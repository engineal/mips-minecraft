...
scoreboard players operation physical_address mem = address mem
...
execute if score address mem matches ..-1610612737 run scoreboard players operation physical_address mem -= 2^31 constants

# Unmapped addresses in kseg1 0xA0000000-0xBFFFFFFF
# Strip off leading 3 bits to map to physical addresses 0x00000000-0x1FFFFFFF
execute if score address mem matches -1610612736..-1073741825 run scoreboard players add physical_address mem 1610612736
...
execute if score debug mem matches 2.. run tellraw @p [{"text":"virtual address: "},{"score":{"name":"address","objective":"mem"}},{"text":" -> physical address: "},{"score":{"name":"physical_address","objective":"mem"}}]
